...
model {
  ...
  // Random-Walk on beta over years
  ...
  // Likelihood
  {
    vector[N] log_lambda;
    for (n in 1:N) {
      log_lambda[n] = log_pop[n] + alpha[state_idx[n]] + beta[year_idx[n]] * age_centered[n];
    }
...
generated quantities {
  ...
  for (n in 1:N) {
    real log_lambda = log_pop[n] + alpha[state_idx[n]] + beta[year_idx[n]] * age_centered[n];
