...
model {
  ...
  {
    vector[N] log_lambda;
    for (n in 1:N) log_lambda[n] = log_pop[n] + alpha_st[state_idx[n], year_idx[n]] + beta_age * age_centered[n];;
...
generated quantities {
  ...
  for (n in 1:N) {
    real log_lambda = log_pop[n] + alpha_st[state_idx[n], year_idx[n]] + beta_age * age_centered[n];;
