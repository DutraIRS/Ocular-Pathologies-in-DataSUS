...
parameters {
  ...
  vector[N_years] beta;              // Time-varying global age effects
...
model {
  ...
  beta[1] ~ normal(0, 2);
  for (t in 2:N_years) beta[t] ~ normal(beta[t-1], sigma_beta_rw);
  for (t in 2:N_years) alpha_st[, t] ~ normal(alpha_st[, t-1], sigma_alpha_rw);

  {
    vector[N] log_lambda;
    for (n in 1:N) log_lambda[n] = log_pop[n] + alpha_st[state_idx[n], year_idx[n]] + beta[year_idx[n]] * age_centered[n];
...
generated quantities {
  ...
  for (n in 1:N) {
    real log_lambda = log_pop[n] + alpha_st[state_idx[n], year_idx[n]] + beta[year_idx[n]] * age_centered[n];
