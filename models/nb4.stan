...
model {
  ...
  {
    vector[N] log_lambda;
    for (n in 1:N) log_lambda[n] = log_pop[n] + alpha[state_idx[n]] + beta_st[state_idx[n], year_idx[n]] * age_centered[n];
...
generated quantities {
  ...
  for (n in 1:N) {
    real log_lambda = log_pop[n] + alpha[state_idx[n]] + beta_st[state_idx[n], year_idx[n]] * age_centered[n];
