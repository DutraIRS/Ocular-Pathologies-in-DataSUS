...
parameters {
  ...
  vector[N_states] beta;            // State-specific age effects
...
model {
  ...
  {
    vector[N] log_lambda;
    for (n in 1:N) log_lambda[n] = log_pop[n] + alpha_st[state_idx[n], year_idx[n]] + beta[state_idx[n]] * age_centered[n];
...
generated quantities {
  ...
  for (n in 1:N) {
    real log_lambda = log_pop[n] + alpha_st[state_idx[n], year_idx[n]] + beta[state_idx[n]] * age_centered[n];
