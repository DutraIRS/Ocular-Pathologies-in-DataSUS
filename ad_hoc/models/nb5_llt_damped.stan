...
parameters {
  ...
  matrix[N_states, N_years] delta_st;
  real beta_age;
  real<lower=0> phi;
  real<lower=0> sigma_alpha_rw;
  real<lower=0> sigma_delta_rw;
  real<lower=0, upper=1> phi_damp;
}
...
model {
  ...
  phi ~ normal(0, 10);
  sigma_alpha_rw ~ exponential(1);
  sigma_delta_rw ~ exponential(2);
  phi_damp ~ beta(8, 2);
  beta_age ~ normal(0, 2);

  for (t in 2:N_years) {
    alpha_st[, t] ~ normal(alpha_st[, t-1] + phi_damp * delta_st[, t-1],
                           sigma_alpha_rw);
    delta_st[, t] ~ normal(phi_damp * delta_st[, t-1], sigma_delta_rw);
  }
