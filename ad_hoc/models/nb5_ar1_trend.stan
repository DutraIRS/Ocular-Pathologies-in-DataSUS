...
parameters {
  ...
  real delta;                                // global linear trend (slope)
...
model {
  ...
  // Stationary initial: alpha[1] ~ trend[1] = mu_s
  alpha_st[, 1] ~ normal(mu_s, sigma_alpha / sqrt(1 - square(rho)));

  for (t in 2:N_years) {
    real trend_prev = delta * (t - 2);
    real trend_now  = delta * (t - 1);
    alpha_st[, t] ~ normal(
      mu_s + trend_now
        + rho * (alpha_st[, t-1] - mu_s - trend_prev),
      sigma_alpha
    );
  }
